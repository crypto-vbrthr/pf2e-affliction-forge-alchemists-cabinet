import {
  ALCHEMISTS_CABINET_CONTENT_VERSION,
  ALCHEMISTS_CABINET_DEFINITIONS,
  ALCHEMISTS_CABINET_MODULE_ID
} from "./content.js";

const MODULE_ID = ALCHEMISTS_CABINET_MODULE_ID;
const CORE_MODULE_ID = "pf2e-affliction-forge";
const PROVIDER_ID = "alchemists-cabinet";
const LIBRARY_ID = "alchemists-cabinet.core";
const PACK_NAME = "affliction-forge-alchemists-cabinet";
...
async function createManagedPack() {
  ...
  try {
    return await CompendiumCollection.createCompendium({
      name: PACK_NAME,
      label: localize("PF2E_AFFLICTION_AC.Module.Library", "Alchemist's Cabinet"),
      type: "Item",
      package: "world",
      packageName: "world",
      system: "pf2e"
...
function addonFlags() {
  return {
    managed: true,
    contentVersion: ALCHEMISTS_CABINET_CONTENT_VERSION,
...
function registerProvider(api) {
  ...
  api.providers.register({
    id: PROVIDER_ID,
    label: localize("PF2E_AFFLICTION_AC.Module.Title", "PF2E Affliction Forge: Alchemist's Cabinet"),
    moduleId: MODULE_ID,
    version: ALCHEMISTS_CABINET_CONTENT_VERSION,
    metadata: {
      themes: ["alchemy", "laboratory", "affliction"],
    ...
    libraries: [{
      ...
      description: localize("PF2E_AFFLICTION_AC.Module.LibraryDescription", "Original alchemical afflictions with semantic Creature Forge tags and weapon-coating support."),
      metadata: {
        themes: ["alchemy", "laboratory", "affliction"],
        semanticTagContract: "1.0.0",
        creatureForgeReady: true,
...
async function initialize(api) {
  ...
  console.info(`${MODULE_ID} | Content synchronized`, {
    ...result,
    total: ALCHEMISTS_CABINET_DEFINITIONS.length,
    collection: PACK_COLLECTION,
    version: ALCHEMISTS_CABINET_CONTENT_VERSION
  });
}
...
Hooks.once("pf2eAfflictionForgeReady", (api) => {
  initialize(api).catch((error) => {
    ...
    globalThis.ui?.notifications?.error?.(
      localize("PF2E_AFFLICTION_AC.Module.SyncFailed", "Alchemist's Cabinet could not synchronize its library. See console.")
    );
  });
});
